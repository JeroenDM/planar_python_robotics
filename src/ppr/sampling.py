...
class SolutionPoint:
    ...
    def add_redundant_joint_samples(self, robot, n=10, method="random"):
        if method == "random":
            ...
            if self.q_fixed_samples is None:
                ...
                self.q_fixed_samples = np.vstack((self.q_fixed_samples, new_samples))
        elif method == "halton":
            if self.q_fixed_samples is None:
    ...
    def calc_joint_solutions(
        self, robot, tp_discrete, check_collision=False, scene=None
    ):
    ...
    def add_joint_solutions_grid_sampling(
        self, robot, check_collision=False, scene=None
    ):
        """ Convert a cartesian trajectory point to joint space """
...
def iterative_bfs(robot, path, scene, tol=0.001, red=10, max_iter=10):
    ...
    for i in range(max_iter):
        ...
        for sp in sol_pts:
            sp.add_joint_solutions_grid_sampling(
                robot, check_collision=True, scene=scene
            )
    ...
    for cart_vec in cart_traj:
        ...
        for cart_pt in cart_vec:
            if method == "grid":
                ...
                sol = robot.ik(
                    cart_pt, q_fixed_samples=q_fixed_samples, scale_samples=True
                )
            if sol["success"]:
                for qsol in sol["q"]:
                    if check_collision:
...
def cart_to_joint_dynamic(
    ...
    if return_sample_info:
        ...
        return (
            [sp.get_joint_solutions() for sp in sol_pts],
            [t_samples, c_samples, n_total],
        )
    else:
