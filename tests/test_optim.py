...
def test_q_derivatives():
    q = np.array([[0, 0], [1, 2], [3, 4], [7, 2]])
    dq_desired = np.array([[10.0, 20.0], [15.0, 20.0], [30.0, 0.0], [40.0, -20.0]])
    ddq_desired = np.array(
        [[50.0, 0.0], [100.0, -100.0], [125.0, -200.0], [100.0, -200.0]]
    )
...
def test_q_derivatives_with_known_function():
    ...
    np.testing.assert_allclose(dx_test[10:40, :], dx[10:40, :], rtol=1, atol=0.01)
    np.testing.assert_allclose(ddx_test[10:40, :], ddx[10:40, :], rtol=1, atol=0.01)
...
sc1 = [Rectangle(3, 1.3, 2, 1, -0.1), Rectangle(3, 0.5, 2, 0.3, 0)]
...
class TestConstraints:
    def test_collision_ieq_con(self):
        ...
        rects = [Rectangle(0, 0, 1, 1, 0), Rectangle(5, 0, 1, 2, 0.3)]
        q_test = [np.array([np.pi / 4, -np.pi / 4, 0.0])]
        actual = collision_ieq_con(q_test, robot, rects)
        desired = np.array(
            [-1.0353553, 1.7665554, 0.4849242, 0.0384695, 0.4849242, -1.0532455]
        )
